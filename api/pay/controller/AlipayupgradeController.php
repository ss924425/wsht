...
class AlipayupgradeController extends RestBaseController
{
    ...
    public function getorderinfo()
    {
        try {
            ...
            if ($user_info['vip_type'] > 1) {
                echo json_encode(['status' => 0, 'msg' => "您已经升级过了~"]);
                exit;
            }
            ...
            $params['version'] = "1.0";
            $params['notify_url'] = "http://" . $_SERVER['HTTP_HOST'] . "/api" . url('pay/Alipayupgrade/notify');
            $params['sign_type'] = $this->sign_type;
            $params['biz_content'] = json_encode([
                'body' => '升级VIP',
                'subject' => '升级VIP',
                'out_trade_no' => $order_info['order_sn'],
                'total_amount' => $order_info['payprice'],
    ...
    public function notify()
    {
        try {
            ...
            if ($check) {
                ...
                if ($order_info['payprice'] != number_format($total_amount, 2)) {
                    return;
                }
                $morder = db('upgrade_order');
                $order_info = $morder->where(['order_sn' => $out_trade_no])->find();
