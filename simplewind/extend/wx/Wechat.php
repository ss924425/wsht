...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function checkAuth($appid = '', $appsecret = '', $token = '')
    {
        ...
        $expires_time = $this->getCache("{$key}_time");
        ...
        if ($result) {
            ...
            $this->setCache($key, $this->access_token);
            $this->setCache("{$key}_time", time() + $expire);
            return $this->access_token;
        }
        return false;
    }
    ...
    public function getMenu()
    {
        ...
        echo 'expires_time:'.date('Y-m-d H:i:s',$expires_time);
        */
