    ...
    function verify($data, $sign, $rsaPublicKeyFilePath, $signType = 'RSA') {
        ...
        if (!$this->checkEmpty($this->alipayPublicKey)) {
            ...
            if ($rsaPublicKeyFilePath){
                $pubKey1 = $rsaPublicKeyFilePath;
                $pubKey = file_get_contents($pubKey1);
            } else {
                $pay_setting = cmf_get_option('pay_setting');
                $this->aliset = $pay_setting['alipay'];
                $pubKey = file_get_contents(cmf_get_image_preview_url($this->aliset['rsa_public_key'])); //支付宝公钥
            }
