...
//        }
        ...
        $strSaveFilePath = './upload/' . $arrInfo["file_path"];
        $this->watermarkPicture($strSaveFilePath);
    ...
    public function watermarkPicture($strSaveFilePath)
    {
        if(!file_exists($strSaveFilePath)){
            return false;
        }

        $watermark_setting = cmf_get_option('watermark_setting');

        if(!$watermark_setting){
           return false;
        }
        $watermark_setting = db('option')->where('option_name','watermark_setting')->value('option_value');
        $watermark_setting = json_decode($watermark_setting,true);

        $image = \think\Image::open($strSaveFilePath);
        file_put_contents('file.txt',json_encode($image).PHP_EOL,FILE_APPEND);
        $width = $image->width();
        $height = $image->height();
        if($width<$watermark_setting['width'] || $height<$watermark_setting){
            return false;
        }
        $position = $watermark_setting['option'] ? $watermark_setting['option'] : 9;
        $opcity = $watermark_setting['opcity'] ? $watermark_setting['opcity'] : 50;
        $watermark = './'.$watermark_setting['src'];
        $image->water($watermark,$position,$opcity)->save($strSaveFilePath . '01');
        @unlink($strSaveFilePath);
        @rename($strSaveFilePath . '01',$strSaveFilePath);
        return true;
    }
}
