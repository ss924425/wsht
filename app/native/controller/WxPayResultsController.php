 ...
 *
 */
class WxPayResultsController extends WxPayDataBaseController
{
    /**
     * 生成签名 - 重写该方法
    ...
    public function CheckSign($config)
    {
        ...
        if(!$this->IsSignSet()){
            file_put_contents('qm.txt', '错误' . PHP_EOL, FILE_APPEND);
            throw new $e("签名错误！");
        }

        $sign = $this->MakeSign($config, false);
        if($this->GetSign() == $sign){
//            file_put_contents('qm.txt', '正确' . PHP_EOL, FILE_APPEND);
            //签名正确
            return true;
        }
        throw new $e("签名错误！");
