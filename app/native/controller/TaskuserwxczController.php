...
class TaskuserwxczController extends AdminBaseController
{
    ...
    function order()
    {
        ...
        $notify_url = "http://" . $_SERVER['HTTP_HOST'] . url('native/Nativenotify/taskusernotify');
        ...
        if (trim($_POST['type']) == 2) {
            $input->SetGoods_tag("保证金充值");
        }
//        $input->SetNotify_url("http://paysdk.weixin.qq.com/notify.php");
        ...
        if ($result){
            ...
            $clorder = [
                ...
                'type' => trim($_POST['type']), //充值类型 1余额 2保证金
                'out_trade_no' => $input->GetOut_trade_no(),
                'cz_type' => 2, // 支付类型 1支付宝 2微信支付
            ];
            db('task_user_order')->insert($clorder);
        }
